...
<div class="container-fluid">
    <div class="row">
        <main class="col-lg-10 col-md-12 mx-auto px-md-4">
            ...
            <!-- Recent Shipments -->
            <div class="card shadow-sm mb-4" id="recent-shipments">
                <div class="card-header bg-light">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="card-title mb-0"><i class="bi bi-box-seam me-2"></i><?= __('recent_shipments') ?></h5>
                        <a href="index.php?page=shipment&customer=<?= urlencode($customer['name']) ?>" class="btn btn-sm btn-outline-primary">
                            <?= __('view_all_shipments') ?>
                        </a>
                    </div>
                </div>
                <div class="card-body">
                            ...
                            <tbody>
                                ...
                                <?php else: ?>
                                    <?php foreach ($shipments as $shipment): ?>
                                        <tr>
                                            <td>
                                                <a href="index.php?page=shipment&action=view&id=<?= $shipment['id'] ?>" class="fw-bold text-decoration-none">
                                            ...
                                            <td class="text-end"><?= number_format($shipment['weight'], 2) ?> <?= __('kg') ?></td>
                                            <td>
                                                <?php 
                                            ...
                                            <td class="text-center">
                                                <div class="btn-group">
                                                    <a href="index.php?page=shipment&action=view&id=<?= $shipment['id'] ?>" class="btn btn-sm btn-outline-secondary" title="<?= __('view') ?>">
                                                        <i class="bi bi-eye"></i>
                                                    </a>
                                                    <?php if ($shipment['status'] !== 'delivered' && $shipment['status'] !== 'cancelled'): ?>
                                                        <a href="index.php?page=shipment&action=edit&id=<?= $shipment['id'] ?>" class="btn btn-sm btn-outline-primary" title="<?= __('edit') ?>">
                                                            <i class="bi bi-pencil"></i>
                                                        </a>
                                                    <?php endif; ?>
                                                    <a href="index.php?page=shipment&action=track&id=<?= $shipment['id'] ?>" class="btn btn-sm btn-outline-info" title="<?= __('track') ?>">
                ...
                <?php if (($totalShipmentPages ?? 0) > 1): ?>
                    <div class="card-footer">
                        <nav aria-label="Shipment page navigation">
                            <ul class="pagination justify-content-center mb-0">
                                <li class="page-item <?= ($shipmentPage ?? 1) <= 1 ? 'disabled' : '' ?>">
                                    <a class="page-link" href="index.php?page=customer&action=view&id=<?= $customer['id'] ?>&shipment_page=<?= ($shipmentPage ?? 1) - 1 ?>&invoice_page=<?= $invoicePage ?? 1 ?>">
                                        <i class="bi bi-chevron-left"></i>
                                    </a>
                                </li>
                                
                                <?php for ($i = 1; $i <= ($totalShipmentPages ?? 1); $i++): ?>
                                    <li class="page-item <?= $i === ($shipmentPage ?? 1) ? 'active' : '' ?>">
                                        <a class="page-link" href="index.php?page=customer&action=view&id=<?= $customer['id'] ?>&shipment_page=<?= $i ?>&invoice_page=<?= $invoicePage ?? 1 ?>">
                                            <?= $i ?>
                                        </a>
                                    </li>
                                <?php endfor; ?>
                                
                                <li class="page-item <?= ($shipmentPage ?? 1) >= ($totalShipmentPages ?? 1) ? 'disabled' : '' ?>">
                                    <a class="page-link" href="index.php?page=customer&action=view&id=<?= $customer['id'] ?>&shipment_page=<?= ($shipmentPage ?? 1) + 1 ?>&invoice_page=<?= $invoicePage ?? 1 ?>">
                                        <i class="bi bi-chevron-right"></i>
                                    </a>
                                </li>
